/* pull only the latest data from reach */
select Guid, InboundCampaign, max(Createtime) as 'Createtime', Product, LeadSource,
...
/* pull also from lmc_reach_feedback */
union 
select `Reach guid`, InboundCampaign, max(Createtime) as 'Createtime', Product, LeadSource,
		`Group`, Callcenter, First_Name, Mobile, Definition, `Attempt DATE`
from `xx_playground`.adm_reach_feedback as F9
) as F6
having
